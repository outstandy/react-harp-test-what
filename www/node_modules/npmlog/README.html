<!DOCTYPE html><html><title>Hometown</title><link rel="stylesheet" href="/scss/main.css"><script src="https://fb.me/react-0.14.7.js"></script><script src="https://fb.me/react-dom-0.14.7.js"></script><script src="https://npmcdn.com/babel-core@5.8.38/browser.min.js"></script><script src="https://npmcdn.com/jquery@3.1.0/dist/jquery.min.js"></script><script src="https://npmcdn.com/remarkable@1.6.2/dist/remarkable.min.js"></script><body><nav><div class="grid"><div class="row"><div class="grid-1-1"><a href="#" class="titling type-300 pink">Hometown</a></div></div></div></nav><h1>npmlog</h1><p>The logger util that npm uses.</p>
...
<h1>Basic Usage</h1><pre><code>var log = require(&#39;npmlog&#39;)
...
<h2>log.enableColor()</h2><p>Force colors to be used on all messages, regardless of the output
stream.</p>
<h2>log.disableColor()</h2><p>Disable colors on all messages.</p>
<h2>log.enableProgress()</h2><p>Enable the display of log activity spinner and progress bar</p>
<h2>log.disableProgress()</h2><p>Disable the display of a progress bar</p>
<h2>log.enableUnicode()</h2><p>Force the unicode theme to be used for the progress bar.</p>
<h2>log.disableUnicode()</h2><p>Disable the use of unicode in the progress bar.</p>
<h2>log.setGaugeTemplate(template)</h2><p>Set a template for outputting the progress bar. See the <a href="https://npmjs.com/package/gauge">gauge documentation</a> for details.</p>
<h2>log.setGaugeThemeset(themes)</h2><p>Select a themeset to pick themes from for the progress bar. See the <a href="https://npmjs.com/package/gauge">gauge documentation</a> for details.</p>
<h2>log.pause()</h2><p>Stop emitting messages to the stream, but do not drop them.</p>
<h2>log.resume()</h2><p>Emit all buffered messages that were written while paused.</p>
<h2>log.log(level, prefix, message, ...)</h2><ul>
...
<h2>log[level](prefix, message, ...)</h2><p>For example,</p>
<ul>
...
<h2>log.addLevel(level, n, style, disp)</h2><ul>
...
<h2>log.newItem(name, todo, weight)</h2><ul>
...
<h2>log.newStream(name, todo, weight)</h2><p>This adds a new <code>are-we-there-yet</code> stream tracker to the progress tracker. The
object returned has the <code>log[level]</code> methods but is otherwise an
<code>are-we-there-yet</code> <code>TrackerStream</code> object.</p>
<h2>log.newGroup(name, weight)</h2><p>This adds a new <code>are-we-there-yet</code> tracker group to the progress tracker. The
object returned has the <code>log[level]</code> methods but is otherwise an
<code>are-we-there-yet</code> <code>TrackerGroup</code> object.</p>
<h1>Events</h1><p>Events are all emitted with the message object.</p>
<ul>
...
<h1>Style Objects</h1><p>Style objects can have the following fields:</p>
<ul>
...
<h1>Message Objects</h1><p>Every log event is emitted with a message object, and the <code>log.record</code>
...
<h1>Blocking TTYs</h1><p>We use <a href="https://npmjs.com/package/set-blocking"><code>set-blocking</code></a> to set
...
so that it can allow output to stdout and stderr to be interlaced.</p><footer><div class="grid"><div class="row"><div class="grid-1-1"><p class="text off-white">This the footer</p></div></div></div></footer><script src="bundle.js"></script></body></html>